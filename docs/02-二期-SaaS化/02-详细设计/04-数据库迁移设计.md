...
> 状态：批次 A 及敏感文件公开地址收口已在本地数据库执行；啤酒币批次 B 已在本地执行，全局价格、订单、钱包、流水和赛事结算表已落地；裁判招募批次 C 仍按计划待实施
> 执行脚本：`docs/sql/20260824_saas_organization_baseline.sql`
> 文件隔离脚本：`docs/sql/20260824_saas_file_scope_backfill.sql`、`docs/sql/20260824_saas_file_orphan_quarantine.sql`
> 最后执行：2026-08-29
...
erDiagram
    ...
    organizer_application ||--o| organizer : approved_as
...
| 表 | 核心字段 | 作用 |
| --- | --- | --- |
| `organizer_application` | `application_no`、主体资料、联系人、材料、`status`、审核信息、账号发放时间/方式、`organizer_id`、`initial_admin_user_id` | 第三方主办方入驻申请、审核和账号发放记录 |
...
`organizer_application` 的状态建议为：

```text
SUBMITTED → UNDER_REVIEW → NEED_MORE_INFO → UNDER_REVIEW
                         ↘ APPROVED → ACCOUNT_ISSUED
                         ↘ REJECTED
```

申请提交时不创建后台账号。审核通过先将申请置为 `APPROVED`；账号发放事务再创建企业账户、`TENANT` 主办方组织、初始 `ORGANIZER_ADMIN` 账号和 `organizer_member`，成功后回写申请单关联 ID 并置为 `ACCOUNT_ISSUED`。账号发放事务失败可重试，不能留下半套组织或账号。初始密码只生成一次并以哈希保存，申请表、操作日志和普通日志不得保存明文密码；账号首次登录必须修改密码。

申请编号必须唯一。账号发放需要锁定申请单，并使用申请单级幂等键或唯一约束，避免重复请求生成多个组织或管理员。联系人手机号等隐私字段按现有 `PiiService` 规则保存密文和哈希。审核历史使用 `admin_operation_log` 记录，目标类型为 `ORGANIZER_APPLICATION`。
...
| `must_change_password` | 初始账号或管理员重置密码后，首次登录必须修改密码 |
| `initial_credential_issued_time` | 初始用户名和一次性密码的交付时间；不保存明文密码 |
...
| 表 | 核心字段 | 作用 |
| --- | --- | --- |
| `beer_coin_product` | `id`、`product_code`、`name`、`status`、`effective_time`、`version_no`、创建人 | 唯一全局价格设置；名称、状态和修订号为内部兼容及审计字段 |
| `beer_coin_product_tier` | `product_id`、起始数量、结束数量、单枚价格、排序 | 连续阶梯价格，最后一档可为开放区间 |
| `beer_coin_purchase_order` | `id`、`order_no`、`enterprise_account_id`、`product_id`、`product_snapshot_json`、`quantity`、`amount`、`status`、`out_trade_no`、`wechat_transaction_id`、`paid_time` | 平台向主办方销售啤酒币的独立支付订单；保存下单时价格快照 |
...
- `beer_coin_product`：全局编码唯一；业务仅维护一条 `ACTIVE` 价格记录，保存时原地更新并递增内部修订号。
...
| 表 | 变更 | 建议索引 |
| --- | --- | --- |
| `organizer_application` | 新增入驻申请、审核状态、审核结果和开通关联 | `unique(application_no)`；`(status, submitted_time, id)`；`organizer_id` |
| `competition` | 新增 `organizer_id`，迁移后非空 | `(organizer_id, status, registration_deadline)` |
| `admin_user` | 新增后台身份、初始改密标记和凭据交付时间 | `(admin_type, status)`；默认组织按实际查询决定 |
...
1. 新增 `organizer_application`、`enterprise_account`、`organizer`、`organizer_member`。
...
7. `ACCOUNT_ISSUED` 申请均能关联一个有效的 `organizer`、初始 `admin_user` 和 `organizer_member`。
8. `REJECTED` 或 `NEED_MORE_INFO` 申请不会产生可登录的主办方管理员账号。
```
