...
## Frontend Headings and Copy

- 页面只保留能表达当前内容的主标题，禁止使用“大标题 + 小标题/副标题/眉题/引导段落”的组合层级。
- 不添加没有实际业务信息的引导、描述、装饰性提示或开发者视角文案。
- 表单字段标签、状态、错误信息和完成业务所需的规则说明可以保留。
- 每次进行前端页面开发或修改前，必须先读取并遵守本节约束。
