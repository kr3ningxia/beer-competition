...
export function competitionAction(competition, loggedIn = false) {
  if (!canSubmitEntry(competition)) {
    return { label: competition?.status === 'PUBLISHED' ? '查看历史结果' : '查看赛事详情', to: `/portal/events/${competition.id}` }
  }
  if (!loggedIn) {
...
export function entryPrimaryAction(entry) {
  ...
  if (entry.status === 'PENDING_PAYMENT' || entry.paymentStatus !== 'PAID') {
    return { label: '查看付款说明', to: '/portal/payment' }
  }
...
export function entryTimeline(entry) {
  ...
  const stored = Boolean(entry?.storedAt) || entry?.status === 'STORED' || entry?.status === 'RESULT_PUBLISHED'
  return [
    { label: '提交资料', done: true, hint: entry?.submittedAt || '已提交' },
    { label: '付款确认', done: paid, hint: paid ? (entry?.paidAt || '已确认') : '等待主办方确认付款' },
    { label: '标签可用', done: paid, hint: paid ? '可下载并贴在酒瓶或外箱' : '付款确认后开放下载' },
    { label: '酒样入库', done: stored, hint: stored ? (entry?.storedAt || '已入库') : '等待主办方收样确认' },
...
export function nextActionText(entry) {
  ...
  if (entry.status === 'REGISTERED') {
    return entry.storedAt ? '等待评审' : '下载标签并送样'
  }
  if (entry.status === 'STORED') {
    return '等待结果发布'
  }
