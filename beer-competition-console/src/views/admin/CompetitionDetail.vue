...
async function refreshRoundProgress(force = false) {
  ...
  try {
    ...
    loadedSectionCompetitionIds.rounds = currentCompetitionKey()
...
async function loadActiveTabData(tab) {
  ...
  if (tab === 'analysis') {
    await loadCompetitionAnalytics()
  }
}

function resetLoadedSections() {
  Object.keys(loadedSectionCompetitionIds).forEach((key) => {
    loadedSectionCompetitionIds[key] = ''
  })
}
...
async function loadResultsWorkspace() {
  ...
  loadedSectionCompetitionIds.results = competitionKey
}
...
<style scoped src="./CompetitionDetail.css"></style>
